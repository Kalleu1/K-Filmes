...
<div style="min-height: 100vh; padding: 4rem 2rem; display: flex; flex-direction: column; align-items: center; justify-content: center; background: #0c0c0e; color: #eaeaea; font-family: system-ui, sans-serif;">
    <div style="background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.06); border-radius: 20px; padding: 3rem; max-width: 500px; width: 100%; box-shadow: 0 20px 40px rgba(0,0,0,0.5); text-align: center;">
        <h1 style="font-family: 'Varela Round', sans-serif; font-size: 2rem; color: #ffffff; margin-top: 0; margin-bottom: 1rem; letter-spacing: -0.02em;">
            Testar Seletor de Pôster
        </h1>
        <p style="color: #a0a0a5; font-size: 0.95rem; margin-bottom: 2rem; line-height: 1.5;">
            Insira o ID do filme da TMDB para buscar os pôsteres oficiais. Exemplos:<br>
        ...
        <div id="result-box" style="display: none; border-top: 1px solid rgba(255,255,255,0.08); padding-top: 2rem; text-align: center;">
            ...
            <div style="display: flex; justify-content: center;">
                <img id="result-poster-img" src="" alt="Poster Selecionado" style="max-width: 180px; border-radius: 12px; box-shadow: 0 10px 25px rgba(0,0,0,0.5); border: 1px solid rgba(255,255,255,0.1);">
            </div>
        </div>
    </div>
</div>

{{-- Inclui o componente Blade do seletor de pôsteres --}}
<x-poster-selector />
@endsection
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (btn) {
        btn.addEventListener('click', () => {
            ...
            // Instancia o componente PosterSelector
            const selector = new window.PosterSelector();
            
            // Abre o seletor com o callback para capturar a seleção
            selector.open(tmdbId, null, (posterPath) => {
                resultPath.textContent = posterPath;
                resultImg.src = `https://image.tmdb.org/t/p/w500${posterPath}`;
